#!/usr/bin/env python3
"""
Genera graficas descriptivas de Fase 2 (perfil del corpus) para la SLR.

Salida:
  Recursos/SRL/sintesis/figuras_fase2/*.png
"""
...
def plot_prisma_fases(prisma: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    ax.set_title("Flujo PRISMA por Fase (N y % sobre Identificados)")
...
def plot_year_distribution(dataset: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    ax.set_title("Distribucion Temporal de Estudios Incluidos")
...
def plot_study_type(dataset: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    ax.invert_yaxis()
    ax.set_title("Tipos de Estudio en el Corpus Incluido")
...
def plot_retrieval_status(screened: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    ax.set_title("Estado de Recuperacion de Texto Completo (N=49)")
...
def plot_quality_band(qa: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    ax.set_title("Bandas de Calidad Metodologica (Kitchenham)")
...
def plot_pollutants_profile(dataset: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    if not counts:
        ...
        ax.set_axis_off()
        ax.set_title("Frecuencia de Contaminantes Reportados (D1)")
...
def plot_nr_fields(dataset: pd.DataFrame, out_dir: Path, colors: List[Tuple[float, float, float, float]]) -> None:
    ...
    if nr_counts.empty:
        ...
        ax.set_axis_off()
        ax.set_title("Completitud de Campos Analiticos")
    ...
    ax.invert_yaxis()
    ax.set_title("Campos Analiticos con Mayor Frecuencia de NR")
...
def main() -> None:
    ...
    synthesis_dir = script_dir.parent
    assets_dir = synthesis_dir / "assets"
    if not assets_dir.exists():
        assets_dir = script_dir / "assets"
    out_dir = synthesis_dir / "figuras_fase2"
    cleanup_previous_pdfs(out_dir)

    dataset = read_csv(assets_dir / "sintesis" / "dataset.csv")
    prisma = read_csv(assets_dir / "sintesis" / "prisma.csv")
    screened = read_csv(assets_dir / "screening" / "01_screening" / "articles_screened.csv")
    qa = read_csv(assets_dir / "screening" / "03_calidad" / "articles_quality_assessment.csv")
