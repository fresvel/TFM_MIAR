...
def plot_architecture_distribution(df: pd.DataFrame, out_dir: Path, lang: str) -> None:
    ...
    if lang == "es":
        ax.set_title("Distribucion de Arquitectura")
        ax.set_xlabel("Numero de estudios")
        ax.set_ylabel("")
    else:
        ax.set_title("Architecture Distribution")
...
def plot_processing_distribution(df: pd.DataFrame, out_dir: Path, lang: str) -> None:
    ...
    if lang == "es":
        ax.set_title("Distribucion de Modo de Procesamiento")
        ax.set_xlabel("Numero de estudios")
        ax.set_ylabel("")
    else:
        ax.set_title("Processing Mode Distribution")
        ax.set_xlabel("Number of studies")
        ax.set_ylabel("")
...
def main() -> None:
    ...
    out_tab = {
        "es": script_dir / "salidas" / "tablas" / "d3" / "es",
        "en": script_dir / "salidas" / "tablas" / "d3" / "en",
    }
    out_fig = {
        "es": script_dir / "salidas" / "figuras" / "d3" / "es",
        "en": script_dir / "salidas" / "figuras" / "d3" / "en",
    }
